...
//#include <ctype.h>
#include "lib.h"
const static unsigned char hextbl[] = "0123456789ABCDEF";
char *sio_write(char*, int);
...
void flush_line(char *data, int index) {
	unsigned int	i, n;
	unsigned char	c;

	n = strlen(data);
	c = 8;
	for(i = index;i < n;i++) sio_write(&(data[i]), 1);
	sio_write(" ", 1);
	for(i = index;i < n;i++) sio_write(&c, 1);
}

void insert_char(char *data, int index, char c) {
	int	i, n;

	n = strlen(data);
	for(i = n;i >= index;i--) data[i + 1] = data[i];
	data[index] = c;
}

void delete_char(char *data, int index) {
	int	i, n;

	n = strlen(data);
	for(i = index;i <= n;i++) data[i - 1] = data[i];
}

int read_string(char *data, int size) {
	...
	while(1) {
		...
		if(c == 2 && index > 0) {
			...
			sio_write(&c, 1);
		} else if(c == 6 && data[index] != 0) {
			flush_line(data, index++);
			sio_write(&c, 1);
		} else if(c == 8 && index > 0) {
			delete_char(data, index);
			c = 8;
			sio_write(&c, 1);
			flush_line(data, --index);
			sio_write(&c, 1);
		} else if(c == 0x7f) {
			delete_char(data, index + 1);
			flush_line(data, index);
			c = 8;
			sio_write(&c, 1);
		} else if(c >= 0x20 && ((index < size) || size == -1)) {
			insert_char(data, index, c);
			flush_line(data, index++);
		}
	}
	n = strlen(data);
...
static int outstring(char *data, int len, char *ptr, int offset, int _num) {
	...
	if(ptr == (char*)-1) sio_write(data, siz);
